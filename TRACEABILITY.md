...
| Test cases | `docs/test-cases/*.md`                 | Nine feature areas currently covered    |
...
| 7.5 통합 Review Queue   | SRS-FR-043 ~ 046                          | `feature/synchronization/synchronization.feature`; `feature/review-queue/review-queue.feature`; `feature/mvp-acceptance/mvp-acceptance.feature` | `docs/test-cases/manual-sync-collection-engine.md`; TC-SYNC-022 ~ 042; `docs/test-cases/review-queue.md`; TC-QUEUE-001 ~ 006; TC-QUEUE-IPC-001 ~ 003; TC-QUEUE-PRELOAD-001; TC-QUEUE-UI-001 ~ 005; TC-QUEUE-SCREEN-001 ~ 003; TC-QUEUE-LIVE-001 | `src/main/services/database/sync-persistence.ts`; `src/main/services/review/index.ts`; `src/main/ipc/review-queue.ts`; `src/preload/index.ts`; `src/renderer/src/composables/useReviewQueue.ts`; `e2e/review-queue.screen.spec.ts`; SQLite tests cover create/update/merge; Review Queue service, IPC, preload, renderer state-model, and mock-preload Playwright screen tests cover read-only full active queue display; live Electron/Notion display remains unverified | Partially Verified |
| 7.6 오늘 복습 목록      | SRS-FR-050 ~ 054                          | `feature/today-review/today-review.feature`                                                                                                     | `docs/test-cases/today-review.md`; TC-REVIEW-001 ~ 017; TC-REVIEW-IPC-001 ~ 003; TC-REVIEW-UI-001/002/011/012                                                                                                                                   | `src/main/services/review/index.ts`; `src/main/ipc/today-review.ts`; `src/main/ipc/review-exclusion.ts`; `src/preload/index.ts`; `src/renderer/src/composables/useTodayReview.ts`; `src/renderer/src/composables/useReviewExclusion.ts`; backend, IPC, preload, renderer state-model, Source filter, exclusion, and database tests pass; live Electron/Notion display remains unverified                                                                                  | Partially Verified |
...
| 7.8 문서 뷰어           | SRS-FR-060 ~ 062; SRS-NFR-SEC-001 ~ 005   | `feature/document-viewer/document-viewer.feature`                                                                                               | `docs/test-cases/document-viewer.md`; TC-VIEWER-001 ~ 010; TC-VIEWER-UI-001 ~ 003                                                                                                                                                               | `src/main/services/document-viewer/index.ts`; `src/main/ipc/document-viewer.ts`; `src/preload/index.ts`; `src/renderer/src/composables/useDocumentViewer.ts`; embedded WebContentsView URL policy, Electron `ERR_*` load-failure tolerance after attachment, bounds validation/resize, default internal open, IPC, preload, and renderer state-model tests pass; live internal Notion rendering remains unverified                                                        | Partially Verified |
...
| 13.3 MVP 인수           | Cross-feature MVP requirements            | `feature/mvp-acceptance/mvp-acceptance.feature`                                                                                                 | `docs/test-cases/mvp-acceptance.md`; TC-MVP-001 ~ 012                                                                                                                                                                                           | MVP acceptance release gate is defined; supporting automated suites pass for collection, sync, Today Review, viewer, rating, status pages, IPC/preload, typecheck, tests, and build; final live Electron MVP pass remains to be recorded                                                                                                                                                                                                                                  | Partially Verified |
...
| Full Review Queue live display is incomplete             | Verify active due and future items appear in the Electron `전체 큐` view              |
...
- Today Review renderer now loads SQLite-backed items through restricted IPC instead of hardcoded
  ...
  remains unverified.
- Full Review Queue now has SRS-FR-046, dedicated Feature/TC coverage, restricted
  `review-queue:list` IPC, narrow `reviewQueue.list` preload exposure, renderer state-model
  coverage, and a mock-preload Playwright screen test for read-only active queue display. Live
  Electron/Notion display remains unverified.
- `docs/test-cases/review-queue.md` now separates service, IPC/preload, renderer state, Playwright
  screen, and live Electron acceptance cases with concrete fixtures, steps, expected results, and
  evidence mapping.
