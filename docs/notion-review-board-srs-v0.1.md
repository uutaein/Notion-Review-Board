...
3. Database 입력이 정확히 하나의 Data Source로 해석되면 해당 Data Source ID로 정규화한다.
4. Database 입력에서 여러 Data Source가 발견되면 자동 선택하지 않고 등록을 거부한다.
5. 사용자가 등록할 Data Source URL 또는 ID를 직접 입력하도록 안내한다.
6. 같은 normalized Notion Target의 활성 Source가 있으면 중복 경고를 표시하고 저장하지 않는다.
...
4. 단독 참조 Item에는 `archive`, `delete`, `keep-history` 정책을 제공한다.
5. `archive`는 전체 Item 메타데이터를 유지하고 status를 `archived`로 전환한다.
6. `delete`는 최소 식별 메타데이터를 가진 `deleted` tombstone을 유지한다.
7. `keep-history`는 운영 Review Item을 제거하고 Review Log snapshot만 유지한다.
8. 모든 정책은 기존 Review Log를 보존한다.
9. Source 삭제와 Item 참조 변경은 하나의 transaction으로 처리한다.
10. `orphaned` 상태와 `system-deleted` sentinel Source는 사용하지 않는다.
...
#### SRS-FR-046 — 전체 Review Queue 조회

앱은 사용자가 Today Review와 별도로 전체 active Review Queue를 확인할 수 있어야 한다.
...
1. 오늘 복습 여부와 관계없이 future dueAt 항목도 표시한다.
2. `changed`, `missing`, `deleted`, `sync_error`, `archived` 항목은 전체 active 큐에서 제외한다.
3. 기본 정렬은 `dueAt` 오름차순이다.
4. 항목의 제목, Source, 분류/태그, 출처, dueAt, lastReviewedAt, lastSyncedAt을 표시한다.
5. 여러 Source가 병합된 항목은 Source 참조 정보를 확인할 수 있어야 한다.
6. FSRS 내부 상태, 토큰, raw Notion 응답, stack trace는 renderer에 노출하지 않는다.
...
| title | 표시 제목 |
| primarySourceId | 대표 Source. 활성 Source 참조가 없는 archived/deleted Item은 null 허용 |
...
| sourceId | 평가 당시 대표 Source ID snapshot. Source 삭제 후에도 보존하며 활성 FK에만 의존하지 않음 |
| sourceName | 평가 당시 Source 표시명 snapshot |
| category | 평가 당시 분류 |
| createdAt | 생성 시각 |
...
`orphaned`는 ReviewItem 상태로 사용하지 않는다. Source 삭제로 활성 Source 참조가 없어지는
Item은 선택 정책에 따라 `archived`, `deleted`, 또는 Item 제거 + Review Log snapshot 보존으로
처리한다.
...
### 11.1.1 전체 Review Queue 화면

영역:

1. 좌측: 전체 active Review Item 목록
2. 우측 또는 상세 영역: 선택한 Review Item의 Source, 분류/태그, 일정, 동기화 정보

수용 기준:

1. Today Review와 별도로 접근할 수 있어야 한다.
2. 미래 dueAt인 active 항목도 표시해야 한다.
3. 변경, 누락, 삭제, 오류, 보관 항목은 별도 상태 화면 또는 정책에 따른 화면에서 다루며 전체 active 큐에는 표시하지 않는다.
4. 전체 큐 조회는 읽기 전용이어야 하며 Review Log, dueAt, FSRS 상태를 변경하지 않는다.
...
| 7.5 통합 Review Queue | SRS-FR-043 ~ 046 |
...
| SRS-OPEN-002 | Notion Database와 Data Source 처리 | Resolved by ADR-015: Data Source 우선, 복수 Data Source는 등록 거부 | Closed |
| SRS-OPEN-003 | 삭제 확정 기준 | missing → 재확인 후 deleted | 몇 회 누락 또는 어떤 API 응답을 확정 기준으로 볼지 |
| SRS-OPEN-004 | Source 삭제 시 Review Log 보존 | Resolved by ADR-015: 모든 Source 삭제 정책에서 보존 | Closed |
...
5. 사용자가 전체 active Review Queue를 확인할 수 있다.
6. 여러 Source의 페이지가 하나의 Today Review 목록으로 표시된다.
7. 동일 Notion Page ID는 중복 생성되지 않는다.
8. 사용자가 Notion 페이지를 내부 뷰어 또는 외부 브라우저로 열 수 있다.
9. 사용자가 다시/어려움/보통/쉬움 평가를 저장할 수 있다.
10. 평가 후 Review Log와 FSRS 기반 next dueAt이 저장된다.
11. 변경된 페이지와 삭제/누락 후보 페이지를 별도 화면에서 확인할 수 있다.
12. 태그 없는 페이지는 `미분류`로 표시된다.
13. Notion 토큰과 민감 정보가 로그와 UI에 노출되지 않는다.
14. 주요 요구사항에 대한 단위/통합/인수 테스트가 통과한다.
